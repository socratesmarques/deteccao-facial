...
class EyeLandmarks:
    def __init__(self, model_path=LBF_MODEL):
        self.status = 'Aguardando rosto para medir os olhos.'
    ...
    def measure(self, frame, face):
        self.status = "Leitura ocular OK"
        height, width = frame.shape[:2]
        x, y, w, h = (int(v) for v in face[:4])
        if min(w, h) < 80 or x < 0 or y < 0 or x+w > width or y+h > height:
            self.status = "Rosto pequeno ou cortado; aproxime-se e centralize."
            return None
        ...
        scale = min(1.0, 384 / max(roi.shape[:2]))
        if scale < 1:
        ...
        if separation < w*scale*0.2 or abs(centers[0][1]-centers[1][1]) > separation*0.4:
            self.status = "Posição dos olhos inconsistente; olhe de frente."
            return None
        ratios = (eye_aspect_ratio(points[36:42]), eye_aspect_ratio(points[42:48]))
        if any(v is None for v in ratios):
            self.status = "Olhos pequenos demais na imagem; aproxime-se."
            return None
        return ratios
